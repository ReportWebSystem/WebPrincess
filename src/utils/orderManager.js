...
class OrderCommand {
    ...
    //rderCommand의 execute 메서드에서 주문이 완료된 후 상품의 수량을 감소시키도록 함.
    execute() {
        if (this.product.quantity <= 0) {
            console.log(`Order failed: ${this.product.name} is out of stock`);  //상품의 수량이 0 이하로 떨어질 경우 주문이 실패하도록 설정.
            return;
        }
        console.log(`Order executed: ${this.user.name} ordered ${this.product.name}`);
